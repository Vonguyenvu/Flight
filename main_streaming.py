...
from config import kafka_spark_options, POSTGRES_JDBC_URL, postgres_properties, validate_config

CHECKPOINT_PATH = "./checkpoints/flight_states"
BRONZE_TABLE = "flight_states"

# Khớp đúng cấu trúc bảng flight_states trong schema.sql
flight_schema = StructType([
    StructField("event_time", StringType(), True),  # parse sang TimestampType ở bước sau
...
def main():
    ...
    query = (
        ...
        .trigger(availableNow=True)   # chạy hết data hiện có trong Kafka rồi dừng
        .start()
    )
